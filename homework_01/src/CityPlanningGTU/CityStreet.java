...
package CityPlanningGTU;

import java.util.Scanner;

/**
 * 
 * CityStreet class provides a buildable two dimensional, and two sided space for CityBuilding type buildings.
 * @version 1.0 28.02.2022
 * @author Burak Kocausta
 * 
 */
public class CityStreet implements Cloneable {
     ...
     */ 
    public CityStreet ( int lengthOfStreet ) throws Exception {
    ...
    /**
     * Overriden toString method returns information about CityStreet object as a String.
    ...
    public boolean equals ( Object other ) {
        if ( other != null && other instanceof CityStreet ) {

            final CityStreet otherStreet = ( CityStreet ) other;
    ...
    public Object clone ( ) {
        ...
        try {
            CityStreet copy = ( CityStreet ) super.clone();
    ...
    public void printSkyline ( ) {
        ...
        for ( int i = 0; i < this.numOfBuildingFront; ++i) {
            if ( this.frontBuildings[i].getHeight() > maxHeight )
                maxHeight = this.frontBuildings[i].getHeight();
        }

        for ( int i = 0; i < this.numOfBuildingBack; ++i ) {
            if ( this.backBuildings[i].getHeight() > maxHeight )
                maxHeight = this.backBuildings[i].getHeight();
        }

        for ( int y = maxHeight; y >= 0; --y ) {
            for ( int x = 0; x <= this.lengthOfStreet; ++x ) {

                boolean flag = false;
                for ( int i = 0; i < this.numOfBuildingFront; ++i ) {
                    
                    boolean conflictFlag = false;
                    int frontBegin = this.frontBuildings[i].getPosition();
                    int frontEnd = this.frontBuildings[i].getLength() + frontBegin;

                    if ( ( this.frontBuildings[i].getHeight() > y )
                            && ( (frontBegin == x) || (frontEnd == x) ) ) {

                        for ( int j = 0; j < this.numOfBuildingBack; ++j ) {

                            int backBegin = this.backBuildings[j].getPosition();
                            int backEnd = this.backBuildings[j].getLength() + backBegin;

                            if ( this.backBuildings[j].getHeight() > y
                                    && !( (backBegin > x) || ( backEnd < x ) ) ) {
                                    if ( backBegin == x || backEnd == x ) {
                                        if ( backEnd == frontBegin || frontEnd == backBegin )
                                            conflictFlag = true;
                                        else
                                            conflictFlag = false;
                                    }
                    ...
                    else if ( ( this.frontBuildings[i].getHeight() == y )
                            && !( (frontBegin > x) || ( frontEnd < x ) ) ) {
                        ...
                        for ( int j = 0; j < this.numOfBuildingBack; ++j ) {
                            ...
                            if ( this.backBuildings[j].getHeight() > this.frontBuildings[i].getHeight()
                                    && !( (backBegin > x) || (backEnd < x) ) ) {
                                conflictFlag = true;
                            }   
                        }
                ...
                if ( !flag ) {
                    ...
                    for ( int i = 0; i < this.numOfBuildingBack; ++i ) {
                        boolean conflictFlag = false;
                        int backBegin = this.backBuildings[i].getPosition();
                        int backEnd = this.backBuildings[i].getLength() + backBegin;

                        if ( ( this.backBuildings[i].getHeight() > y )
                                && ( (backBegin == x) || ( backEnd == x ) ) ) {

                            for ( int j = 0; j < this.numOfBuildingFront; ++j ) {

                                int frontBegin = this.frontBuildings[j].getPosition();
                                int frontEnd = this.frontBuildings[j].getLength() + frontBegin;

                                if ( this.frontBuildings[j].getHeight() > y
                                        && !( (frontBegin > x) || ( frontEnd < x ) ) ) {
                                    
                                    if ( frontBegin == x || frontEnd == x ) {
                                        if ( backEnd == frontBegin || frontEnd == backBegin )
                                            conflictFlag = true;
                                        else
                                            conflictFlag = false;
                        ...
                        else if ( ( this.backBuildings[i].getHeight() == y )
                                        && !( (backBegin > x) || ( backEnd < x ) ) ) {
                            ...
                            for ( int j = 0; j < this.numOfBuildingFront; ++j ) {
                                ...
                                if ( this.frontBuildings[j].getHeight() > this.backBuildings[i].getHeight()
                                        && !( ( frontBegin > x) || ( frontEnd < x ) ) ) {
                                    conflictFlag = true;
                                }   
                            }
